...
for( i in 1:length(chr) ){
	...
	# print(nrow(get(paste("data_", chr[i], sep=""))))
}
...
write.table(datb, file=paste(output_file,"_BED.tlx",sep=""), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
write.table(datc, file=paste(output_file,"_rmBED.tlx",sep=""), sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)