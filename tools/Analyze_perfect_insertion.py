...
Usage:
    Analyze_perfect_insertion <sequence> <chromosome> <cutsite> <tab_file> <strand>

Options:
<sequence>              must be an x nt sequence, x must be even number, 5'->3', and cutsite at x/2|x/2+1
<chromosome>            chromosome of your target
...
def one_insertion(sequence=None,chromosome=None,cutsite=None,tab_file=None,strand=None):
...
def main():
    ...
    kwargs = {'sequence':args['<sequence>'], 'cutsite':args['<cutsite>'], 'chromosome':args['<chromosome>'],'tab_file':args['<tab_file>'], 'strand':args['<strand>']}
    print('[PEM-Q-TOOL] sequence: ' + str(kwargs['sequence']))
    print('[PEM-Q-TOOL] chromosome: ' + str(kwargs['chromosome']))
