...
class Define_indel(object):
    ...
    def generate_indel_tab_file(self):
        ...
        cutoff_condition = (indel_transloc[indel_s] <= self.cutsite + 5) & \
                            (indel_transloc[indel_e] >= self.cutsite - 5)
        indel_transloc_cutoff = indel_transloc[cutoff_condition]
        germline_addup = indel_transloc[~(cutoff_condition)]
        
        # # merge germline file
        # germline = pd.read_table("raw/" + self.basename + "_Germline.tab",sep = '\t')
        # germline = germline.append([germline_addup])
        # germline.to_csv("raw/"+self.basename + "_Germline.tab", header = True, sep = '\t', index=False)
