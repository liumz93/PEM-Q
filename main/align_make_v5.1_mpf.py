...
class Align_make(object):
    ...
    def align_genome(self): 
        ...
        if not os.path.exists(chek_file):
            ...
            #-a -A 2 -B 8 -O 8 -E 4 -T 50 -k 20 -p -M 
            cmd = "bwa mem -Y -t 4 -A 2 -B 8 -O 8 -E 4 -T 50 -k 20 -p -M {} {} > bwa_align/{} 2>bwa_align/bwa_alignstich.log".format(bwa_index_path, 
                                                 bwa_fq_file,
                                                 self.sam)
            os.system(cmd)
            print("[PEM-Q] "+cmd)
            
            cmd = "samtools view -S -b -h -q 30 bwa_align/{} > bwa_align/{} \
