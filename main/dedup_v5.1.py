...
def dedup(basename=None):
    ...
    indel_cutoff_dedup = indel_cutoff_merge.drop_duplicates(['Rname','Strand','Bait_end','Junction','Barcode'],keep='first')
    insertion_sv_dedup = insertion_sv_merge.drop_duplicates(['Rname','Strand','Bait_end','Junction','Insertion','Barcode'],keep='first')
    ...
    insertion_sv_dedup.to_csv("unique/"+basename + "_Insertion_sv.tab", header = True, sep = '\t', index=False)
    ...
    indel_cutoff_dedup.to_csv("unique/"+basename + "_smallindel_cutoff.tab", header = True, sep = '\t', index=False)
