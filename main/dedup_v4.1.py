...
def dedup(basename=None):
    ...
    sid = pd.read_csv(basename+"_SID_all.tab",sep = '\t')
    sv = pd.read_csv(basename+"_SV.tab",sep = '\t')
    substitution = pd.read_csv(basename+"_Substitution.tab",sep = '\t')
    insertion = pd.read_csv(basename+"_Insertion.tab",sep = '\t')
    deletion = pd.read_csv(basename+"_Deletion.tab",sep = '\t')
    germline = pd.read_csv(basename+"_Germline.tab",sep = '\t')
    ...
    sid_dedup = sid_merge.drop_duplicates(['Rname','Strand','Junction','Barcode'],keep='first')
    sv_dedup = sv_merge.drop_duplicates(['Rname','Strand','Junction','Barcode'],keep='first')
    insertion_dedup = insertion_merge.drop_duplicates(['Rname','Strand','Junction','Insertion','Barcode'],keep='first')
    deletion_dedup = deletion_merge.drop_duplicates(['Rname','Strand','Junction','Microhomolog','Barcode'],keep='first')
    
    germline_dedup = germline_merge.drop_duplicates(['Cigar','Barcode'],keep= 'first')
